...
\newcommand\hlt[1]{\colorbox{yellow}{\textcolor{black}{#1}}}
...
%$\text{mode}(\eta|\theta=\theta_0)=\theta_0$ was changed to \colorbox{yellow}{$argmax_{\eta} \pi(\eta|\theta=\theta_0)=\eta_0$}. 
...
% \textcolor{black}{...which to develop Complex Innovative Designs (CIDs) – a performance goal set forth under PDUFA VI legislation\citep{FDA2017}}.
%

{For traditional frequentist approaches, prior information (e.g., historical data) is often used to provide external evidence about the magnitude of a plausible, clinically meaningful 
...
%
%\textcolor{black}{The potential use of Bayesian methods is discussed throughout the draft guidance \citep{FDA_CID}, created in response to the 21st Century CURES Act \citep{USCongress2016}, including the use of Bayesian methods to extrapolate information from adult patients to pediatric settings as one type of possible CID (see Section III, Part C)}.
...
%	%
%\textcolor{black}{General feedback: Compare this text against the previous version. There was a good deal of 
...
%
%\textcolor{black}{All of the appendices need to be moved to Supplemental Online Appendices in the format of the Biometrics paper I sent. If not, they are 
...
%
%\textcolor{black}{Please make sure the first appendix touches on this point.}
%
...
%\textcolor{black}{Note that I am revising this section significantly. Predominately this is due to imprecise language and what
...
{Stopping criteria are based on whether the posterior probability that the treatment effect is in a particular region is sufficiently large. For region $\Theta_i$, this is formalized as}
...
%\textcolor{black}{Note that I am changing the notation to what I think will make it more understandable but the graphs need to change too. I also struggle
...
the data which is defined (in our case) using the \textcolor{black}{enthusiastic prior}.
	%
{The prior-predictive distribution for replicated data denoted by $\mathbf{D}_{\text{rep}}$ reflects the probability of observing this hypothetical data given 
...
{\psi({\textcolor{black}{\mathbf{D}}})=\int {p(\textcolor{black}{\mathbf{D}_{\text{rep}}})}  1[p(\textcolor{black}{\mathbf{D}_{\text{rep}}})\leq p(\textcolor{black}{\mathbf{D}})] d(\textcolor{black}{\mathbf{D}_{\text{rep}}})}
%\sum_{\mathbf{D}}p^{(h)}(\mathbf{D})1[p^{(h)}(\mathbf{D})\leq p^{(h)}(\textcolor{black}{\mathbf{D}})]
\end{equation}
%which in the case of discrete data is equal to $\psi({\textcolor{black}{\mathbf{D}}})=\sum_{\mathbf{D}}p(\mathbf{D})1[p(\mathbf{D})\leq p(\textcolor{black}{\mathbf{D}})]$.
%
...
%Note that being well-supported by \eqref{eq:pred_dist} implies the likelihood $p(\mathbf{D})$ is consistent with $\pi(\theta)$.
}
%Box's p-value can be interpreted as the probability of observing data as or more extreme than $\textcolor{black}{\mathbf{D}}$, given the predictive distribution. 
%
%Small values of $\psi({\textcolor{black}{\mathbf{D}}})$ indicate a lack of compatibility or congruency between the prior and the data. 
%
...
%
%If $\psi^{(E)}(\textcolor{black}{\mathbf{D}})>\psi^{(S)}(\textcolor{black}{\mathbf{D}})$, then the observed data are more consistent with the enthusiastic prior, which should be given a greater weight in the mixture. Similarly, if $\psi^{(S)}(\textcolor{black}{\mathbf{D}})>\psi^{(E)}(\textcolor{black}{\mathbf{D}})$ then the skeptical prior should be given a greater mixing weight.
%A conservative choice of mixing weight which gives full weight to the skeptical prior (i.e. $\omega=1$) whenever $\psi^{(S)}(\textcolor{black}{\mathbf{D}})\geq \psi^{(E)}(\textcolor{black}{\mathbf{D}})$ is
%\begin{equation}\label{eq:adaptive_prior}
%\omega = 1 - \text{max}\left(0, \psi^{(E)}(\textcolor{black}{\mathbf{D}})-\psi^{(S)}(\textcolor{black}{\mathbf{D}})\right).
%%\omega=\begin{cases} 
%%      1 & \text{if } \psi^{(S)}(\textcolor{black}{\mathbf{D}})\geq \psi^{(E)}(\textcolor{black}{\mathbf{D}}) \\
%%      1-\left[ \psi^{(E)}(\textcolor{black}{\mathbf{D}})-\psi^{(S)}(\textcolor{black}{\mathbf{D}}) \right] &\text{if } \psi^{(S)}(\textcolor{black}{\mathbf{D}})< \psi^{(E)}(\textcolor{black}{\mathbf{D}})
%%   \end{cases}
%\end{equation}
%%
...
%\omega=\text{max}\left(\delta, \psi^{(S)}(\textcolor{black}{\mathbf{D}})-\psi^{(E)}(\textcolor{black}{\mathbf{D}})\right).
%\end{equation}
%This liberal choice of mixing weight gives $1 - \delta$ weight to the enthusiastic prior whenever the data are shown to be more compatible with the enthusiastic prior, that is, when $\psi^{(E)}(\textcolor{black}{\mathbf{D}})\geq \psi^{(S)}(\textcolor{black}{\mathbf{D}})$. This formulation of the mixing weight will always include some of the skeptical component in the mixture through the quantity $\delta$, which is typically in the range of $(0, 0.25)$.
%}
%
...
%\textcolor{black}{This section confuses me a good deal. In reference to the figure (and hypotheses to be tested), what are $\eta_0$ and $\eta_1$. These things have not be defined.
%	%
...
%\textcolor{black}{All of the concepts in this Section are basic and need not take up space in the main paper. I would cut this entire section. Still, I give some comments
...
%\textcolor{black}{I do not understand what this paragraph is saying. Neither the expected sample size or expected trial duration are rigorously defined. In the absence of that, 
...
%
%\textcolor{black}{Make sure this is moved to supplemental appendix and appropriately referenced. See my recent paper in biometrics for examples for how to write references to the online supplementary materials.}
%
...
%
Figure \ref{fig:figure2}(B) shows the results of a trial with early stopping for futility once 30 outcomes are ascertained, and enrollment is henceforth terminated. %The final data matches the last interim analysis since enrolled patients would be transitioned off the IP.
...
%
%\sout{Consequently, the choice of monitoring prior and stoppage criteria are the same regardless of the frequency of data monitoring and the number of patients in %progress at enrollment termination, although these factors do impact the operating characteristics of the trial.}
%
