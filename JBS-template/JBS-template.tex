...
\textcolor{red}{Stopping criteria are based on the probability that the treatment effect is in a particular region $\Theta_i$ given the data $\mathbf{D}$ is greater than the threshold for compelling evidence, that is,
\begin{equation}\label{eq:convinced}
		P_\pi(\theta\in\Theta_i|\mathbf{D}) > 1-\epsilon,
\end{equation} 
where the subscript $\pi$ indicates the probability is calculated based on a prior or posterior distribution, as in equation \eqref{eq:compellingevidence}.
}
...
the data which is defined (in our case) using the \textcolor{black}{enthusiastic prior}.
	%
The prior-predictive distribution for \textcolor{red}{replicated data $\mathbf{D}_{\text{rep}}$ (also called the marginal likelihood) reflects the probability of observing hypothetical data $\mathbf{D}_{\text{rep}}$ given 
...
\textcolor{red}{
\subsection*{C: Step-by-Step Implementation}
\begin{enumerate}
\item 
\end{enumerate}
}
