...
can be consistently applied in a wide range of clinical trial applications. The basis for this framework is the 
...
%
An enthusiastic monitoring prior in the generalized normal family of distributions can have density at the mode equal to $k\times \frac{1}{\sqrt{2\pi}\sigma}$, with $k<1$ indicating a flattened distribution and $k>1$ indicating a peaked distribution at the mode, relative to the default normal distribution. 
%
...
%
This is a rational perspective for a monitoring prior since the enthusiastic viewpoint should reflect substantial uncertainty on the likelihood of values at and around the alternative.
%
...
%%
A locally non-informative prior is shown in Panel D of Figure \ref{fig:figure1}, and the technical definition is in Web Appendix B.
...
The adaptive mixing weight $\omega$ is determined by an assessment of prior-data conflict, proposed by Box \citep{Box1980}, derived using the prior predictive distribution of 
...
%
The marginal-conditional factorization of the joint prior $\pi(\theta,\eta)=\pi(\theta)\times\pi(\eta|\theta)$ allows direct elicitation of the marginal prior on the treatment effect and provides the ability to incorporate prior opinion on the nuisance parameters through their conditional distribution given $\theta$.
%
...
%
The value 0.67 was the observed proportion of responders among adults enrolled in the ACT 1 and ACT 2 trials \citep{Rutgeerts2005} who received the same weight-based dose of 5mg/kg dose (N = 242).
%
...
%
For each instance, the monitoring priors (first set of distributions), the posterior distributions at three selected interim analyses (middle sets), and from the final analysis (last set) are shown.
	%

Panel A of Figure \ref{fig:figure2} shows the results of a trial with early stopping for efficacy once 30 outcomes are ascertained, and enrollment is henceforth terminated. Note that the final data in this example no longer meet the criteria for substantial evidence of efficacy.
	%
...
%
It is of particular interest when the threshold for substantial evidence is satisfied for an interim analysis but is no longer satisfied once outcomes from patients in progress are ascertained, was the case in Panel A of Figure \ref{fig:figure2}.
%
...
%
In many of these cases with early stoppage for efficacy with a true $\theta=\theta_0$, the final posterior probability which includes the total follow-up no longer meets the threshold for efficacy.
\begin{figure}\begin{center}
...
We consider the trial ``The Pediatric Lupus Trial of Belimumab Plus Background Standard Therapy (PLUTO)" (NCT01649765) which was conducted between September 2012 and January 2018.
%
...
%
Data from two studies of belimumab in adults resulted in a placebo response probability of $0.39$, and a 10mg/kg response probability of $0.51$. 
%
Using these values for the null and hypothesized response probabilities for the treatment group and assuming a response probability of 0.39 for the control group, a frequentist two-sided hypothesis test with confidence level $95\%$ and $80\%$ power would require 266 patients.
%
...
%
An estimate for the pediatric response probability is denoted by $\eta_0=0.39$ (i.e. the sample proportion of responders from the pooled adult studies), and for purposes of monitoring, a plausible, clinically meaningful difference in response probabilities is $\theta_1=0.12$ (i.e. anticipating the adult studies treatment response probability of $0.51$) \citep{Travis2019}.
%
