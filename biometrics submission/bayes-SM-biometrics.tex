...
Prior information can also be incorporated into the monitoring priors by using the predictive distribution of the data under the skeptical and enthusiastic priors. Let $\mathcal{L}(\theta,\eta|\mathbf{D})$ be the likelihood for the new trial data at some point in time.

Compute the two predictive distributions as follows
\begin{equation}
p^{(h)}(\mathbf{D})=\int \int \mathcal{L}(\theta,\eta|\mathbf{D})\pi^{(h)}(\theta,\eta)d\theta d\eta
\end{equation}

with $h\in\{S,E\}$ for the skeptical and enthusiastic prior respectively.
Consider $\mathbf{D}_{obs}$ to be the observed data at some point in time. For each predictive density, we compute the following:
\begin{equation}
\psi^{(h)}(\mathbf{D})=\sum_{\mathbf{D}}p^{(h)}(\mathbf{D})1[p^{(h)}(\mathbf{D})\leq p^{(h)}(\mathbf{D}_{obs})]
\end{equation}

This can be interpreted as the probability of observing data as or more extreme given the predictive distribution (either skeptical or enthusiastic predictive distribution).

Based on this, we can set the mixing weight as

\begin{equation}
w^{(E)}(\mathbf{D})=\text{max}(\psi^{(E)}(\mathbf{D})-\psi^{(S)}(\mathbf{D}),0)
\end{equation}

with $w^{(S)}(\mathbf{D})=1-w^{(E)}(\mathbf{D})$.
