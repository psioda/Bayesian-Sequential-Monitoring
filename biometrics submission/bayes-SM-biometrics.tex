...
\begin{abstract}
\tcr{
This introduction is much too generic. The introduction needs to (1) briefly highlight the motivation (using one or two sentences; valuee of sequential monitoring and incorporating external information into the design prospectively), (2) fully describe \textit{all} novel aspects of the paper (this is go a long way to getting the paper accepted or rejected), and (3) how the method is demonstrated in the paper. I have modified the final sentence which I think is good to keep but feel free to tweak.
}

Conclusions from Bayesian clinical trials are most useful when specification of prior distributions are intuitively related to the research objectives. This paper defines monitoring priors using generalized normal distributions which reflect prior belief defined using a standardized definition of compelling level of evidence. The generalized normal distribution adds a layer of flexibility to the foundational normal distribution to reflect prior belief, which has a tangible impact on the operating characteristics of a clinical trial design. \tcb{The proposed method is  demonstrated by designing a simple single-arm proof-of-activity trial in pediatric ulcerative colitis and a two-group randomized controlled trial in pediatric lupus with both designs incorporating information from adult trials to varying degrees.}
\end{abstract}
...
In the clinical development of a therapeutic product, \tcb{often challenges} impede a clinical trial from reaching its \tcb{primary objectives} (e.g. difficulty in enrolling, limited patient populations, long latency in observing the outcome of interest, etc.). In settings where \tcb{pertinent existing data are available, Bayesian designs offer a natural approach for incorporating that information into the design and analysis of future trials}. \tcb{An example where Bayesian methods could be valuable is in the area of pediatric clinical trials where enrollment is often difficult and where relevant data from adult trials are often available}. Utilizing information from the adult trials may result in pediatric trials \tcb{that are more efficient (e.g., increased power, fewer subjects required and/or shorter trial durations)}.

\tcr{This above paragraph should briefly introduce sequential monitoring. As it is, the first paragraph seems to imply Bayesian methods are all about information borrowing but the sequential monitoring aspect is also very important. The total paragraph length should be no longer than twice the current one.}

\vspace{1cm}
\tcr{At this point in the introduction you need to discuss our methodological innovations. What is our method (at a high level) and how can it be helpful in practice. This revised abstract can pull heavily from this section. We cannot get more than a few paragraphs into the paper without telling the reader what we are doing that is novel. You will lose the reviewer! It is critical in this portion that we are attempting to develop a framework for sequential monitoring that can be applied broadly and consistently which may be appealing to regulators. That is a big point.
%
In this section you can take some of what you have below to contrast the Bayesian approach and frequentist approaches but you don't want to dwell on that too much. Flip the order -- focus on Bayes and explain how frequentist methods are different and make sure anything you say about frequentist methods is precisely correct.
For example, I do not think the statement about pre-specified interim analyses / alpha spending functions is correct and we will get slammed if we make false claims. Alpha spending functions were developed to make the rigidity of a fixed number of analyses and fixed timing go away were they not -- read up on this before revising.}

\vspace{1cm}
\tcb{Add material... customize this...
Conclusions from Bayesian inferences are not affected by the frequency with which the data are monitored, and such interim analyses should be encouraged to terminate data collection if appropriate \citep{Berry1989,Berry1993,Spiegelhalter1994}. The Bayesian perspective is based on the likelihood principle which asserts that any data with the same likelihood function should lead to the same conclusion \citep{Berger1988}. In the context of sequential analysis of clinical trials, the stopping rule that leads to the termination of data collection is irrelevant to the conclusion \citep{Barnard1947,Anscombe1963,Cornfield1966a,Cornfield1966b}. In contrast ... add stuff Frequentist group sequential designs rely on pre-specified interim analyses and spending functions to maintain desired trial operating characteristics \citep{Pocock1977,OBrien1979}. }

\tcr{At this point point in the introduction you can be into the recent literature on sequential trials. Where have these methods been used post-2000? You don't need to describe methods in depth but do need to show you have knowledge of what has been done recently (relevant stuff). If there is little that is fine.} 

\vspace{1cm}
\tcr{Much of the text in orange can be wove in above, referenced elsewhere in the paper or cut.}

\vspace{0.25cm}
\tcr{ General advice...
The last sentence in the orange text illustrates a style of writing you want to try and avoid when writing statistical papers. The entire paragraph describes an approach for doing something and then the last sentence says we use that approach. The better strategy is to say exactly what we are doing and give credit through name or citation to ideas that first came from others}
...
\vspace{1cm}
\tcr{As noted above, the below type of paragraph needs to cover very early in the paper and needs to be specific about what are are doing. This is very general and also not perfectly precise. For example, I'm not sure the framework addresses how to specify the trial hypotheses really.}

This paper presents a structured framework to determine the major components of a Bayesian sequentially monitored clinical trial, including the trial hypothesis, monitoring priors, and stopping criteria. It is shown that the generalized normal distribution is an intuitive and flexible choice for monitoring prior specification.
