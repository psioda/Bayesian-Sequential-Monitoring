...
and therefore really doesn't add much. These types of sentences almost always lead to reviewer questions.}
...
for these settings.
%
Moreover, in settings where pertinent preexisting data are available, Bayesian methods inarguably provide a natural approach for incorporating 
that information via a prior distribution into the design and analysis of a future trial. 

In this paper, we propose a strategy for designing Bayesian sequentially monitored clinical trials that entails eliciting priors used to monitor 
enrollment or data collection (i.e., monitoring priors) and stopping criteria that can be derived in semi-automatic fashion based on standard inputs
needed for trial planning. These inputs include (1) the null boundary value for the treatment effect, (2) a plausible, clinically meaningful value for 
the treatment effect, and (3) a criteria for defining substantial evidence of efficacy. In principle, the plausible, clinically meaningful value for 
the treatment effect could and should be informed from a relevant external data, when available. 


Skeptical and enthusiastic priors are defined with the generalized normal distribution which is applicable to a wide array of data types. 
