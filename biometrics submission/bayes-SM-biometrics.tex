...
In the clinical development of a therapeutic product, external forces may impede a clinical trial from reaching its objective (e.g. difficulty in enrolling, limited patient populations, long latency in observing the outcome of interest, etc.). In a setting where relevant external data exists, Bayesian designs offer a natural approach to incorporate this external evidence through prior specifications. An example of this is in clinical trials in pediatric populations which are difficult to enroll and for which there is typically relevant data from adult clinical trials. Utilizing information from the adult trials can result in increasing the power to evaluate the effectiveness of the therapeutic product in the pediatric clinical trial. 

%Development of this type of design has been prioritized by the 21st Century Cures Act \citep{USCongress2016} and the Prescription Drug User Fee Act VI \citep{FDA2017}.

Data monitoring also mitigates the impact of external forces on the trial by providing the ability to stop the trial early for efficacy or futility, which has the benefit of saving time and resources. %Group sequential designs rely on these interim analyses and stopping criteria to determine when the trial will be terminated rather than a pre-specified sample size. 
Frequentist group sequential designs rely on pre-specified interim analyses and spending functions to mantain desired trial operating characteristics \citep{Pocock1977,OBrien1979}. Conversely, the conclusions from Bayesian designs are not affected by frequent monitoring of the data, and such interim analyses should be encouraged to terminate data collection if appropriate \citep{Berry1989,Berry1993,Spiegelhalter1994}. The Bayesian perspective is based on the likelihood principle which asserts that any data with the same likelihood function should lead to the same conclusion \citep{Berger1988}. In the context of sequential analysis of clinical trials, the stopping rule that leads to the termination of data collection is irrelevant to the conclusion \citep{Barnard1947,Anscombe1963,Cornfield1966a,Cornfield1966b}. Bayesian sequential designs which incorporate external evidence are the focus of this paper.

%It is most succinctly stated as ``It is entirely appropriate to collect data until a point has been proven or disproven, or until the data collector runs out of time, money, or patience."\citep{Edwards1963}. 
