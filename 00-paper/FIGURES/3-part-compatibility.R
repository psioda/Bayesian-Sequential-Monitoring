...
plot(x, y, xlab = "Observed Response Difference", ylab = "Box's p-value", pch = 19, cex = 0.25, xlim = c(-0.3,0.4))
...
y <- omega.skpt
plot(x,y, pch = 19, cex = 0.25, ylim = c(0,1), xlab = "Observed Response Difference", ylab = "Mixture Weights", xlim = c(-0.3,0.4))
model <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4))
myPredict <- predict( model ) 
ix <- sort(x,index.return=T)$ix
lines(x[ix], myPredict[ix], lwd=2 )  
