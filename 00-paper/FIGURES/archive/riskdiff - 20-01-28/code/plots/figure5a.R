...
grid$z<-prior.skpt.nc(grid$x,grid$y)
...
axis(2,at=mu,labels=c(as.expression(bquote(mu))))
...
lines(c(0,1),c(0,1))
lines(c(0,mu),c(mu,mu))
lines(c(mu,mu),c(0,mu))
...
legend('bottomright',
       legend= c(#as.expression(bquote(mu == .(p.enth))),
         ...
         as.expression(bquote(delta == delta[S]*","~alpha[1] == .(sigma0.skpt)*","~beta[1] == .(lambda0.skpt))),
...
#      at=c(mu),
#      labels=c(as.expression(bquote(mu))))
# segments(y0=mu,x0=0,x1=skpt.y[mu*(x.len-1)+1])

# par(mar=c(0,0,0,0),
#     fig=c(0.45,0.9,0.3,0.31), # use scaling based on layout()
