enth_tail_area <- function(){
  ...
  for (i in 1:length(alpha0.seq)){
    ...
    if (i%%10 == 0){print(
      integrate_debug(enth.prior.1, xmin = 0,  xmax = 0.5, ymin = 0, ymax = function(x) 1 - x) +  ### XMAX SHOULD BE 1
      integrate_debug(enth.prior.2, xmin = -0.5, xmax = 0, ymin = function(x) -x, ymax = 1)   ### BOTTOM PIECE OF INTEGRAL IS TOO TINY!
                                                                                              ### XMIN SHOULD BE -1!
    )}
    }
