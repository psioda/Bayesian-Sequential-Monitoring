marginal <- function(){
  ...
  for (i in 1:length(alpha0.seq)){
    for (j in 1:length(beta0.seq)){
      ...
      result1[i,j] <- integrate(enth.rd.prior, lower = -1,         upper = delta.skpt)$value
      result2[i,j] <- integrate(enth.rd.prior, lower = delta.skpt, upper = delta.intr)$value
    }
    if (i%%10 == 0){print(paste0("Simulation ", i))}
    if (i%%10 == 0){print(integrate(enth.rd.prior, lower = -1, upper = 1)$value)}
  }
