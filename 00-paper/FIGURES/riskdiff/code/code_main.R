...
if (.Platform$OS.type == "unix")    { 
  ...
  # source functions
  source("/nas/longleaf/home/ekwiatko/FDA/riskdiff/code/code_functions.R")
  source("/nas/longleaf/home/ekwiatko/FDA/riskdiff/code/args_model.R")
  
  # using idx to identify simulation
  model<-read.csv(file="/nas/longleaf/home/ekwiatko/FDA/riskdiff/code/args_simulation.csv",header=TRUE,sep=",")
  p.IP<-model$p.IP[idx]
  p.PC<-model$p.PC[idx]
  freq.mntr<-model$freq.mntr[idx]
  enr.shape<-model$enr.shape[idx]
...
names<-c("eff.mon.initial","eff.mon.final",
                ...
                "inference0","inference25","inference50","inference75","inference100")
...
for (k in 1:reps){
  ...
  result.inference<-inference(n.final)
  inner[k,"inference0"]<-result.inference[1]
  inner[k,"inference25"]<-result.inference[2]
  inner[k,"inference50"]<-result.inference[3]
  inner[k,"inference75"]<-result.inference[4]
  inner[k,"inference100"]<-result.inference[5]
}
