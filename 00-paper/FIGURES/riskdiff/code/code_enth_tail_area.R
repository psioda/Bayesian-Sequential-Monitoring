enth_tail_area <- function(){
  ...
  sigma0.seq <- seq(1E-3, 1, length = 1000)
  enth.lambda0 <- 2
  result1 <- matrix(NA, ncol=length(sigma0.seq))
  ...
  assign("enth.sigma0", enth.sigma0, envir = .GlobalEnv)
  assign("enth.lambda0", enth.lambda0, envir = .GlobalEnv)
}