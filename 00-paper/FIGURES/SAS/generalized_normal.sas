...
start den(x) global(skip_data,y,n,mu0,sigma0,lambda0,log_nc);
 if skip_data = 0 then      return exp( y*log(x) + (n-y)*log(1-x) - 0.5*(  (abs(x-mu0)/sigma0)##lambda0 ) - log_nc);
 else if skip_data = 1 then return exp(  - 0.5*(  (abs(x-mu0)/sigma0)##lambda0 ) - log_nc);
  *if skip_data = 0 then      return exp( y*log(x) + (n-y)*log(1-x) + (mu0-1)*log(x) + (sigma0-1)*log(1-x)*lambda0 - log_nc);
  *else if skip_data = 1 then return exp(  (mu0-1)*log(x) + (sigma0-1)*log(1-x)*lambda0 - log_nc);
finish;
