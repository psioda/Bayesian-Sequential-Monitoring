%macro sim_pos(seed=1,nSims=10000,plots=ALL,nSimsPOS=500);
     ...
     do sim = 1 to &nSims.;
		 ...
		 ** perform the sequential analysis;
		 ...
		 do while(stop=0);
		  ...
		  do j = 1 to dim(r);
           ...
           if sim = 1 and round(pi,1e-5) in (0.20,0.40) then output example;
         ...
         ** perform analysis when all outcomes observed;
		  ...
		  do j = 1 to dim(r);
		   ...
		   %pos(pref=fin,nVar=nFin,yVar=yFin);
...
data out.example_pos_&dsout.;
 set example;
run;


data Sim out.sim_pos_&dsout;
 set sim;
