...
H_0&:\theta\leq 0.15\\
H_1&:\theta>0.15\\
\pi_S(\theta)&\sim\mathcal{B}()\\
\pi_E(\theta)&\sim\mathcal{B}()\\
\text{efficacy criteria:}&P(\theta>0.15|\mathbf{D},\pi_S)\geq 0.95\\
\text{futility criteria:}&P(\theta\leq 0.35|\mathbf{D},\pi_E)\geq 0.95\\
\text{exhausted resources:}&N=XX \text{ patient outcomes obtained}
\end{align*}
Make figure similar to slide 19, separate plots for Skeptical Beta Prior and Enthuastic Beta Prior.
Make figure similar to slide 20, but vary to get 5 different priors.
Make examples of violin plots for particular sample paths (slides 21-22).
...
Include 4 different sequential monitoring strategies.
