...
It is intuitive to center the skeptical and enthuastic priors around the quantities $\theta_0$ and $\theta_A$ respectively, so that $E(\pi_S)=\theta_0$ and $E(\pi_E)=\theta_A$. For example, suppose it is desirable that the skeptical prior places small probability $\lambda>0$ that the $\theta\geq\theta_A$, which is the highly clinically relevant treatment effect.
...
%Motivated by a trial for Vemurafenib (\cite{Hyman2015}) let $\theta_0=0.15$ and $\theta_A=0.45$. 
...
The skeptical prior will be Beta distributed, centered around $\theta_0=0.15$, and have $2.5\%$ prior probability that $\theta>\theta_A$. Similarly, the enthuastic prior will be centered around $\theta_A=0.45$ and have $2.5\%$ prior probability that $\theta<\theta_0$.
\newpage
\begin{center}
...
\end{align*}
\newpage
\subsubsection{Example paths (violin plots)}
\begin{center}
\includegraphics[width=3.5in]{D:/Users/ekwiatko/Documents/GitHub/Bayesian-Sequential-Monitoring/00-paper/FIGURES/violin_efficacy.png}
\end{center}
Perhaps reduce the number of interim analyses...after every 10 subjects or so. Have at most 4 analyses before the final result. Nothing wrong with having 4 analyses for both examples.
\begin{center}
\includegraphics[width=3.5in]{D:/Users/ekwiatko/Documents/GitHub/Bayesian-Sequential-Monitoring/00-paper/FIGURES/violin_futility.png}
\end{center}
\newpage
...
\end{center}
\begin{itemize}
\item EFF is the probability of the trial stopping early for efficacy.
\item FUT is the probability of the trial stopping early for futility. 
\item INC is the probability of reaching the maximum sample size without a conclusive monitoring result.
\item CP is the coverage probability using the mixture prior.
\item SS is the average sample size at the interim analysis and at the end of follow-up. Note there are approximately 8 subjects undergoing follow-up when enrollment is terminated.
\item PM is the posterior mean using the mixture prior.
\end{itemize}
\newpage
...
\item Consider hyperlinking formulas. 
\item Still need to fix violin plots!
\end{itemize}
\begin{center}
\includegraphics[width=3in]{D:/Users/ekwiatko/Documents/GitHub/Bayesian-Sequential-Monitoring/00-paper/FIGURES/sequential_design_properties_2}
\includegraphics[width=3in]{D:/Users/ekwiatko/Documents/GitHub/Bayesian-Sequential-Monitoring/00-paper/FIGURES/type1error_0809}
\end{center}
\begin{itemize}
...
\begin{itemize}
\item Test the hypothesis that power curves will be identical and what will change is the expected sample sizes.
\end{itemize}

\textit{Include details for how Bayesian monitoring has good frequentist properties even with frequent interim analyses}.
\begin{center}
\includegraphics[width=3in]{D:/Users/ekwiatko/Documents/GitHub/Bayesian-Sequential-Monitoring/00-paper/FIGURES/evidence_decrease.png}
\end{center}
\begin{itemize}
