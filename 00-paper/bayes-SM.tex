...
\subsubsection{Model formulation \& prior elicitation}
In this section we consider a sequential monitoring approach in a parallel two-group setting using a binary response endpoint. We consider the case where the goal is prove superiority of an investigational product (IP) to a control which could be a placebo. Let $\theta_0$ represent the response rate the control group and $\theta_1$ represent the response probability for the IP group. Let $H_0:\theta_1-\theta_0\leq 0.1$ vs. $H_1: \theta_1-\theta_0>0.1$.

In this setting pessimism or optimism is reflected in the induced prior on the difference in proportions $\theta_1 - \theta_0$. One easy way to specify such a prior in this case is to elicit a bivariate normal prior for $\theta_0$ and $\theta_1$ truncated to the unit square:
\begin{align*}
\pi(\theta_0,\theta_1)\propto& \hspace{0.05in}exp \left\{-\frac{1}{2}\left|\frac{(\theta_1-\theta_0)-\delta}{\sigma}\right|^\alpha\right\}
\end{align*}


Let $\sigma=4$, $\alpha=2$, $\delta_{skpt}=0$, and $\delta_{enth}=0.2$. The skeptic believes there is no difference in response rates by treatment group, and the enthusiastic person believes the IP group will have a response rate probability that is 0.2 higher than the placebo group. 
...
Enrollment will proceed until one of the following three conditions are satisfied:
\begin{align*}
\text{Efficacy criteria (EFF): }&P(\theta_1-\theta_0>0.10|\mathbf{D},\pi_S)\geq 0.95\\
\text{Futility criteria (FUT): }&P(\theta_1-\theta_0 \leq 0.15|\mathbf{D},\pi_E)\geq 0.85\\
\text{Maximum sample size: }&N=50 \text{ patient outcomes obtained per group}
\end{align*}
\newpage
\subsubsection{Example paths}
...
\end{center}		
\newpage
Alternatively, consider a prior for the response rates that is also informative based on an anticipated response rate in the placebo group:
\begin{align*}
\pi(\theta_0,\theta_1)=&\pi(\theta_0)\times\pi(\theta_1|\theta_0)\\
\propto&exp\left\{-\frac{1}{2}\left|\frac{\theta_0-\mu}{\sigma}\right|^{\alpha_0}\right\}\times exp\left\{-\frac{1}{2}\left|\frac{(\theta_1-\theta_0)-\delta}{\sigma_1}\right|^{\alpha_1}\right\}
\end{align*}
